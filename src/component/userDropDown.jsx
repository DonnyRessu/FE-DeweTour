          ...
          if (user.status == true) {
            return (
              localStorage.setItem(
                i,
                JSON.stringify({ ...user, status: false })
              ),
              setIsLogin(false),
                navigate('/'),
    ...
    return (
        ...
        <div className="relative mt-12 left-10">
            ...
            <div className={`absolute w-40 top-full right-7 mt-4 rounded-md py-2 bg-white z-10`}>
                ...
                <Link to={'/payment'} className="flex items-center cursor-pointer gap-2 font-semibold px-5 mb-2">
                    <FaDonate className="text-xl text-yellow-400" />Pay
                </Link>
